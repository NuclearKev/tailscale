...
func init() {
	...
	NetDeviceTunHook.Set(func() error {
		_, err := NetDeviceTun()
		return err
	})
}

func NetDeviceTun() ([]byte, error) {
	return exec.Command("/sbin/NetDevice", "tun").CombinedOutput()
}
...
func diagnoseLinuxTUNFailure(tunName string, logf logger.Logf, createErr error) {
	...
	logf("NetBSD kernel version: %s", kernel)

	NetDeviceOut, err := NetDeviceTun()
	if err == nil {
		logf("'NetBSD tun' successful")
		// Either tun is currently loaded, or it's statically
		// compiled into the kernel (which NetDevice checks
	...
	logf("is CONFIG_TUN enabled in your kernel? `NetBSD tun` failed with: %s", NetDeviceOut)
